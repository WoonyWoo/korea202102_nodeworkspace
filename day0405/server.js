...
var qs = require("querystring"); //(쪼개져서)직렬화된 전송한 데이터에 대한 해석을 담당(문자열로 해석가능함)
...
//1:1 로 대응되는 요청을 처리한다면,,... 개발에 효율성 없고...X
//결론: 정적 자원 처리 뿐만 아니라, 개발의 편의성을 위해 좀 더 개선된 서버 모듈로 전환!!!
function getImage(request, response){
    fs.readFile("./images/star.png", function(err, data){
...
function regist(request, response){
    ...
    request.on("end", function(){
        ...
        con.query(sql, function(e, fields){
            if(e){ //쿼리수행중 심각한 에러 발생
...
function getList2(request, response){
    ...
    con.query(sql, function(err,record,fields){
        //record 변수엔 json들이 1차원 배열에 탑재되어 있다..
        // console.log("record is ", record);
        //파일을 모두 읽으면 익명함수가 호출되고, 이 익명함수 안에 매개변수에, 읽혀진 모든 데이터가
        //매개변수로 전달된다!1
...
//따라서 아래의 방법은 두번 다시는 사용하지 않겠다..(개발 규모가 좀 커질때)
...
function getDetail(request, response){
    ...
    //get방식은 body 를 통해 넘겨지는 post 방식에 비해 header를 타고 전송되어 오므로,
    //추출하기가 용이하다!!(마치 봉투 겉면에 씌여진 글씨와 같다)
    ...
    con.query(sql, function(err, result, fields){
        ...
        fs.readFile("./detail.ejs","utf8", function(err,data){
            if(err){
                ...
                //따라서, 모든 render 작업이 끝나면 html로 재구성하여 응답정보에 싣어서
                //클라이언트에게 응답을 실행한다!!
                response.end(ejs.render(data, {
                    //한 건이라 할지라도, select 문의 겨로가는 배열이다!! 따라서 0번째 추출해서 보내자
                    record:result[0] 
                }));
            }
...
function edit(request, response){
    ...
    //글 수정은 클라이언트로부터 post방식으로 서버에 전송되기 때문에, 그 데이터가 body에 들어있다.
    ...
    request.on("end", function(){
        //이 시점이 파라미터가 하나의 문자열로 복원도니 시점이다!!
        var obj = qs.parse(content);
        ...
        //쿼리문 실행하여 수정을 완료하세요,
        //수정완료 메시지 alert()으로 띄우고, 상세보기 페이지를 다시 보여주기!!
...
function del(request, response){
    ...
    //커넥션 객체는 접속 정보를 가진 객체이며, 이 객체를 통해 쿼리를 수행할 수 있다.. 또한 mysql, 접속을 해제도 가능하다

    //select문의 경우엔 결과를 가져와야 하기때문에 가운데 인수, result인수가 필요하지만
    //delete, update, insert DML은 가져올 레코드가 없기 때무에 인수가 2개면 충분함.
    con.query(sql, function(err, fields){
        if(err){
            console.log(err);
        }else{
