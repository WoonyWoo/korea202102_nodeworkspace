...
효율성이 떨어진다.. 따라서 기본 모듈외에, http모듈을 좀 더 개선한 express모듈을 사용해보자!!
...
//__dirname 전역변수는? 현재 실행중인 js파일의 디렉토리 위치를 반환
...
/*--------------------------------------------------------------------------------
클라이언트의 요청 처리!!! 요청 url에 대한 조건문 X, 정적자원에 대한 처리필요X
...
app.get("/notice/list", function(request, response){
    ...
    //select 문 수행하자!!
    con.query("select * from notice", function(error, result, fields){
        if(error){
            console.log(error); //에러 분석을 위해 콘솔 화면에 로그를 남김
        }else{
            ...
            fs.readFile("./notice/list.ejs","utf8",function(err, data){
                ...
                //클라이언트에 보내기 전에, 서버측에서 먼저 실행을 해버리자!!
...
app.post("/notice/regist", function(request, response){
    ...
    //3) 쿼리실행
    /*
    var sql="insert into notice(title, writer, content) values('"+title+"','"+writer+"','"+content+"')";
    */
    //바인드 변수를 이용하면, 따옴표 문제를 고민하지 않아도 됨. 단 주의!!!
    //바인드 변수의 사용목적은 따옴표 때문이 아니라, DB의 성능과 관련이 있다..(java시간에...)
                ...
                if(error1){
                    ...
                    fs.readFile("./notice/detail.ejs", "utf8", function(error, data){
                        if(error){
                            ...
                            response.writeHead(200, {"Content-Type":"text/html;charset=utf-8"});
                            response.end(ejs.render(data, {
                                //result는 1건이라 할지라도 배열이므로, 배열에서 꺼내서 보내주자
                                record:result[0]
                            }));
                        }
                    });
