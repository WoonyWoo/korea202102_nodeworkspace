...
oracledb.fetchAsString = [oracledb.CLOB]; //clob 데이터를 string으로
...
//이 시점 이후부터는 conStr변수의 값은 변할 수 없다~~(상수화 시킴)
const conStr={
    user:"node",
    password:"node",
                    ...
                    fs.readFile("./views/news/list.ejs", "utf8", function(error, data){
                        if(error){
                            ...
                            var r = ejs.render(data,{
                                ...
                                param:{
                                    page:currentPage,
                                    /*result는 mysql과 다르게 json 객체의 rows 속성에 2차원배열에 들어있다.. */
                                    record:result.rows,
                                    lib:mymodule //ejs가 사용할 수 있도록 모듈 전달
                                }
                            }); //ejs 해석 및 실행하기!!
        ...
        if(err){
            ...
            con.execute(sql, [title, writer, content], function(error, result){
                if(error){
                    ...
                    //여기서도 무조건 등록된다는 보장은 없다.. 즉 오라클에 반영되었느냐 여부는 result를 통해 또 알아봐야 한다..
                    console.log("result는 ",result);
...
app.get("/news/detail", function(request, response){
    ...
    //response.render() 메서드는 기본적으로 views 라는 정해진 디렉토리 안에
    //정해진 뷰 엔진을 찾게된다..(뷰엔진은 개발자가 선택할 수 있다..)
    ...
    oracledb.getConnection(conStr, function(err, con){
        if(err){
            ...
            con.execute(sql, [news_id, msg, author], function(error, result){ //쿼리문 실행
                if(error){
                    console.log("insert 쿼리 실행 중 에러발생", error);
                    ...
                    // response.end(mymodule.getMsgUrl("댓글 등록 성공", "/news/detail?news_id=" + news_id));
                }
                con.close();
            });
        }
