...
class Hero extends GameObject{
    constructor(container, src, width, height, x, y, velX, velY) {
        ...
        this.g=0.5; //중력 가속도 효과를 내기 위한 변수, 이 변수는 상속과 상관없이 내꺼!!
        this.jump = false; //주인공의 점프 상태를 판단할 수 있는 변수(즉 점프하는지 안하는지)
    }
    //히어로는 움직임이 있다! 따라서 메서드 정의가 요구된다
    //그렇지만, 부모에게 물려받은 메서드가, 현재 나의 상황에는 맞지 않을 경우
    //업그레이드할 필요가 있다!!(java, C# 등 oop언어에서는 이러한 메서드 재정의 기법을
    //가리켜 오버라이딩(overriding)이라 한다)

    tick(){
        //코드에서는 보이지 않지만, 현재 클래스는 GameObject의 모든것을 다 가지고
        //있는 것과 마찬가지이다!! 즉 내것처럼 접근할 수 있다
