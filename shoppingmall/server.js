...
var static = require("serve-static"); //정적자원 처리를 위한 모듈 (설치)
...
app.set("view engine", "ejs"); //등록 후엔 자동으로 무조건 views 라는 디렉토리 하위에서
...
app.get("/admin/product/sublist", function(request, response){
    ...
    var con = mysql.createConnection(conStr);
    con.query(sql, [topcategory_id], function(err, result, fields){
        if(err){
